...
public class TTT extends JFrame {
   ...
   ///// stage is for data that is saved in the files
   ...
   public void initGame() {
      for (int row = 0; row < ROWS; ++row) {
         for (int col = 0; col < COLS; ++col) {
            board[row][col] = Seed.E; // all cells E
         }
      }
   ...
   /** Return true if it is a draw (i.e., no more E cell) */
   public boolean isDraw() {
      for (int row = 0; row < ROWS; ++row) {
         for (int col = 0; col < COLS; ++col) {
            if (board[row][col] == Seed.E) {
               return false; // an E cell found, not draw, exit
            }
         }
      }
      return true;  // no more E cell, it's a draw
   }
   ...
   class DrawCanvas extends JPanel {
      ...
      public void paintComponent(Graphics g) {  // invoke via repaint()
         ...
         // Draw the Seeds of all the cells if they are not E
